...
function createBackup() {
  ...
  const backupPath = path.join(CONFIG.BACKUP_DIR, `project-memory-${timestamp}.md`);
...
function addDecisionRecord() {
  ...
  function askQuestion() {
    if (currentQuestion < questions.length) {
      ...
      const [title, background, options, decision, rationale, impact, tags] = answers;
      ...
      // 處理標籤
      const tagsList = tags ? tags.split(' ').map(tag => `#${tag.trim()}`).join(' ') : '';
...
${refList}`;
      ...
      if (tags) {
        const tagsList = tags.split(' ').map(tag => `#${tag.trim()}`).join(' ');
        note += `
**標籤**: ${tagsList}`;
      }
...
function showStatistics() {
  ...
  // 最後修改時間
  const lastModified = stats_file.mtime.toISOString().replace('T', ' ').split('.')[0];
  console.log(`最後修改: ${lastModified}`);
}
