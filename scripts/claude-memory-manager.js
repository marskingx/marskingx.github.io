...
${code}`;
    ...
    const insertPos =
      content.indexOf("```", content.indexOf(commandSection)) + 3;

    const updatedContent =
      content.slice(0, insertPos) + newCommand + content.slice(insertPos);
...
`;
    ...
    const nextSection = content.indexOf(
      "##",
      content.indexOf(solutionSection) + solutionSection.length,
    );
  ...
  async addTodo() {
    ...
    const updatedContent =
      content.slice(0, insertPos) + todoLine + "\\n" + content.slice(insertPos);
  ...
  async updateConfig() {
    ...
    const sectionEnd = content.indexOf(
      "###",
      content.indexOf(section) + section.length,
    );
    const insertPos =
      sectionEnd > 0
        ? sectionEnd
        : content.indexOf("##", content.indexOf(section) + section.length);
  ...
  showStatus() {
    ...
    console.log(
      `📄 檔案大小: ${Math.round((fs.statSync(CLAUDE_MEMORY_PATH).size / 1024) * 100) / 100} KB`,
    );
