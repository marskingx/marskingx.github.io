...
class MultiAIManager {
  constructor() {
    ...
    this.aiAgents = [
      { name: "Claude (克勞德)", branch: "claude-dev", worktree: null },
      {
        name: "Codex",
        branch: "codex-dev",
        worktree: "D:/marskingx-worktrees/codex-dev",
      },
      {
        name: "Gemini",
        branch: "gemini-dev",
        worktree: "D:/marskingx-worktrees/gemini-dev",
      },
    ];
  }
    ...
    for (const ai of this.aiAgents) {
      ...
      try {
        ...
        if (status.clean) {
          ...
          console.log(
            `   ⚠️  有未提交變更: ${status.modified} 修改, ${status.untracked} 新增`,
          );
        }
        ...
        console.log(
          `   ⏰ 最後提交: ${lastCommit.date} (${lastCommit.author})`,
        );
      } catch (_error) {
        console.log(`   ❌ 檢查失敗: ${_error.message}`);
      }
    ...
    for (const ai of this.aiAgents) {
      ...
      try {
        ...
        if (_error.message.includes("CONFLICT")) {
          ...
          console.log(`   ❌ rebase 失敗: ${_error.message.split("\n")[0]}`);
        }
      }
  ...
  async checkConflicts() {
    ...
    for (const ai of this.aiAgents) {
      ...
      try {
        ...
        console.log(`⚠️  檢查 ${ai.name} 失敗: ${_error.message}`);
      }
    }

    // 顯示潛在衝突
    const potentialConflicts = Array.from(conflictFiles.entries()).filter(
      ([_file, agents]) => agents.length > 1,
    );
  ...
  async syncMemoryFiles() {
    ...
    const sharedMemoryPath = path.join(this.projectRoot, "docs/aimemory/shared/ai-shared.md");

    if (!fs.existsSync(sharedMemoryPath)) {
      console.log("❌ 共用記憶檔案 ai-shared.md 不存在");
      return;
    }
