...
class PerformanceAnalyzer {
    ...
    analyzeStaticFiles() {
        ...
        Object.entries(byType)
            ...
            .slice(0, 10)
            .forEach(([ext, data]) => {
                console.log(`   ${ext || 'no-ext'}: ${data.count} 個檔案, ${(data.size / 1024).toFixed(2)} KB`);
            });
        console.log();
    }
    ...
    analyzeServiceWorker() {
        ...
        if (fs.existsSync(swPath)) {
            ...
            console.log(`   ✅ Service Worker 已部署 (${(swSize / 1024).toFixed(2)} KB)`);
    ...
    analyzeCriticalResources() {
        ...
        // 檢查圖片檔案
        const imageFiles = this.findFiles(this.publicDir, /\.(png|jpg|jpeg|webp|svg)$/);
        console.log(`   圖片檔案: ${imageFiles.length} 個`);
        
        // 檢查 WebP 支援
        const webpFiles = this.findFiles(this.publicDir, /\.webp$/);
        const totalImages = this.findFiles(this.publicDir, /\.(png|jpg|jpeg)$/).length;
        console.log(`   WebP 轉換率: ${((webpFiles.length / totalImages) * 100).toFixed(1)}%`);
        console.log();
    }
    ...
    generateRecommendations() {
        ...
        // 檢查圖片優化
        const largeImages = this.findLargeFiles(this.publicDir, 500 * 1024, /\.(png|jpg|jpeg)$/);
    ...
    findLargeFiles(dir, sizeLimit, pattern) {
        ...
        return files.filter(file => 
            file.size > sizeLimit && 
            (pattern ? pattern.test(file.name) : true)
        );
    }
